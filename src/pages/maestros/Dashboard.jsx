...
import {
  ...
  LogOut, UserCheck, X, GraduationCap, MapPin, Sparkles, Filter
} from 'lucide-react';
...
const StatCard = ({ title, value, icon: Icon, color = 'blue', subtitle }) => {
  const styles = {
    blue: { bg: 'bg-blue-50/80 text-blue-600 border-blue-100', text: 'text-blue-600' },
    green: { bg: 'bg-emerald-50/80 text-emerald-600 border-emerald-100', text: 'text-emerald-600' },
    purple: { bg: 'bg-purple-50/80 text-purple-600 border-purple-100', text: 'text-purple-600' },
    amber: { bg: 'bg-amber-50/80 text-amber-600 border-amber-100', text: 'text-amber-600' },
  };

  const currentStyle = styles[color] || styles.blue;

  return (
    <div className="bg-white rounded-2xl border border-slate-200/70 p-5 shadow-xs hover:shadow-md transition-all duration-200 flex items-center justify-between">
      <div className="space-y-1">
        <p className="text-xs font-bold uppercase tracking-wider text-slate-400">{title}</p>
        <p className="text-3xl font-extrabold text-slate-900">{value}</p>
        {subtitle && <p className="text-xs text-slate-500 font-medium">{subtitle}</p>}
      </div>
      <div className={`p-3.5 rounded-2xl border ${currentStyle.bg}`}>
...
const AdminDashboardView = ({ maestros, asignaciones, grupos, onOpenAddMaestro, onOpenAddAsignacion }) => {
  return (
    <div className="space-y-6">
      {/* Resumen Superior */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <StatCard title="Docentes Activos" value={maestros.length} icon={Users} color="blue" subtitle="Profesores registrados" />
      ...
      {/* Acciones Rápidas */}
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/70 shadow-xs">
        <div>
          <h3 className="text-base font-extrabold text-slate-900">Gestión de Cátedras Escolares</h3>
          <p className="text-xs text-slate-500 font-medium mt-0.5">Administra a los docentes titulares y sus materias</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={onOpenAddMaestro}
            className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-2 active:scale-95 cursor-pointer"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-2 active:scale-95 cursor-pointer"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200/70 shadow-xs overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h4 className="font-extrabold text-slate-900 text-base">Asignaciones Actuales</h4>
          <span className="text-xs font-bold bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full border border-indigo-100">
            {asignaciones.length} Activas
          </span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/70 text-slate-400 font-bold uppercase text-[11px] tracking-wider border-b border-slate-100">
                <th className="p-4">Docente Titular</th>
                <th className="p-4">Especialidad / Email</th>
                ...
                return (
                  <tr key={asig.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 text-slate-500 font-medium">
                      <p className="text-xs text-slate-700 font-bold">{maestro?.especialidad || 'General'}</p>
                      <p className="text-[11px] text-slate-400">{maestro?.email || 'sin email'}</p>
                    </td>
                    <td className="p-4 font-bold text-indigo-950">{asig.materia}</td>
                    <td className="p-4">
...
const AdminMaestrosListView = ({ maestros, onDeleteMaestro, onOpenAddMaestro }) => {
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 bg-white p-5 rounded-2xl border border-slate-200/70 shadow-xs">
        <div>
          <h3 className="text-base font-extrabold text-slate-900">Directorio de Profesores</h3>
          <p className="text-xs text-slate-500 font-medium">Lista de personal docente activo en la institución</p>
        </div>
        <button
          onClick={onOpenAddMaestro}
          className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-2 self-start sm:self-auto cursor-pointer"
        >
          <Plus className="w-4 h-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {maestros.map((m) => (
          <div key={m.id} className="bg-white rounded-2xl border border-slate-200/70 p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-indigo-600">{m.especialidad}</p>
                </div>
              </div>
              <button
            ...
            <div className="pt-3 border-t border-slate-100 text-xs text-slate-500 space-y-1">
              <p><strong className="text-slate-700">Email:</strong> {m.email}</p>
              <p><strong className="text-slate-700">Teléfono:</strong> {m.telefono}</p>
            </div>
          </div>
        ))}
      </div>
...
const MaestroResumenView = ({ asignacionesProfesor, maestroNombre }) => (
  <div className="space-y-6">
    <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-3xl p-6 text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        <span className="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-xs font-bold border border-indigo-500/30">
          Docente Activo
        </span>
        <h2 className="text-2xl font-extrabold mt-2">¡Hola de nuevo, {maestroNombre}! 👋</h2>
        <p className="text-slate-300 text-xs sm:text-sm mt-1">Aquí está la información sobre tus cátedras y horarios agendados.</p>
      </div>
    </div>

    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
      <StatCard title="Mis Clases Asignadas" value={asignacionesProfesor.length} icon={BookOpen} color="blue" />
      <StatCard title="Horas Semanales" value="16h" icon={Clock} color="green" />
      <StatCard title="Revisiones Pendientes" value="5" icon={CheckSquare} color="amber" />
    </div>

    <div className="bg-white rounded-2xl border border-slate-200/70 p-6 shadow-xs space-y-4">
      <h3 className="text-base font-extrabold text-slate-900">Mis Asignaturas y Aulas</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {asignacionesProfesor.map((clase) => (
          <div key={clase.id} className="p-4 rounded-xl border border-slate-200/80 hover:border-indigo-200 hover:bg-indigo-50/20 transition-all flex items-center justify-between">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Grupo {clase.grupo} • {clase.aula}</p>
                <p className="text-[11px] text-indigo-600 font-bold mt-0.5">{clase.horario}</p>
              </div>
            </div>
            <button className="px-3 py-1.5 bg-slate-100 hover:bg-indigo-600 hover:text-white text-slate-700 text-xs font-bold rounded-lg transition-colors cursor-pointer">
              Ver Aula
            </button>
          </div>
        ))}
...
const CalendarioView = ({ asignaciones }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-xs flex justify-between items-center">
        <div>
          <h3 className="text-base font-extrabold text-slate-900">Horario Semanal de Cátedras</h3>
          <p className="text-xs text-slate-500 font-medium">Distribución de clases asignadas en la semana</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {dias.map((dia, idx) => (
          <div key={dia} className="bg-white rounded-2xl border border-slate-200/70 p-4 shadow-xs space-y-3">
            <div className="pb-2 border-b border-slate-100 text-center">
              <span className="text-xs font-extrabold uppercase text-slate-400 tracking-wider">{dia}</span>
            </div>
            <div className="space-y-2">
              {asignaciones.slice(idx % asignaciones.length, (idx % asignaciones.length) + 2).map((asig, i) => (
                <div key={i} className="p-3 bg-indigo-50/60 border border-indigo-100 rounded-xl space-y-1">
                  <p className="text-xs font-extrabold text-indigo-950 truncate">{asig.materia}</p>
                  <p className="text-[11px] font-semibold text-indigo-600">Grupo {asig.grupo}</p>
...
const SistemaEscolar = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row text-slate-800 font-sans antialiased">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed md:sticky top-0 left-0 h-screen z-50 bg-white border-r border-slate-200/80 transition-all duration-300 flex flex-col w-64 ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
        }`}
      >
        <div className="h-16 flex items-center justify-between px-5 border-b border-slate-100 shrink-0">
          <span className="font-black text-xl text-slate-900 flex items-center gap-2.5">
            <div className="p-2 bg-indigo-600 text-white rounded-xl shadow-xs">
              <Shield className="w-5 h-5" />
            </div>
            EduControl
          </span>
          <button onClick={() => setSidebarOpen(false)} className="md:hidden p-1.5 rounded-lg text-slate-400 hover:bg-slate-100">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/70">
          {currentUser.role === 'admin' ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white p-3 rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer active:scale-95"
            >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 p-3 rounded-xl text-xs font-bold transition-all cursor-pointer active:scale-95"
            >
      ...
      <div className="flex-1 min-w-0 flex flex-col min-h-screen">
        {/* HEADER TOP */}
        <header className="h-16 bg-white border-b border-slate-200/80 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-8 shrink-0">
          <div className="flex items-center gap-3">
            <button
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-3 border-l pl-3 sm:pl-4 border-slate-200">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-xs text-white shadow-xs ${
                currentUser.role === 'admin' ? 'bg-slate-900' : 'bg-indigo-600'
              }`}>
                {currentUser.role === 'admin' ? 'A' : 'P'}
              </div>
              <div className="hidden sm:block text-left">
                <p className="text-xs font-extrabold text-slate-900">{currentUser.nombre}</p>
                <p className="text-[10px] text-slate-400 font-bold uppercase">{currentUser.role === 'admin' ? 'Administrador' : 'Docente Titular'}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <form onSubmit={handleAddMaestro} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full h-11 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              required
            />
          </div>
          <div>
      ...
      >
        <form onSubmit={handleAddAsignacion} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Docente Titular</label>
            <select
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Aula / Salón</label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Horario Fijo</label>
            <input
              type="text"
